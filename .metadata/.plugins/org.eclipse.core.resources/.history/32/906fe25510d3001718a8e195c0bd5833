...
public class Enemy extends JLabel{
	...
	private int health, wFrame, hFrame, direction = 1, score = 1;
	private double speed, dx, dy;
	private double amp = 5;
	private double freq = .05;
	private ArrayList<Color> colors = new ArrayList<Color>();
	
	//Constructor for enemy class, default move type, default score
	...
	//Constructor for enemy class, default move type, custom score
	public Enemy(String s, int hp, double spd, int scr, int width, int height){
		id = s;
		moveType = "strightDown";
		health = hp;
		speed = spd;
	...
	public Enemy(String s, String m, int hp, double spd, int scr, int width, int height){
		id = s;
		moveType = m;
		health = hp;
		speed = spd;
	...
	public Enemy(String s, String m, int hp, double spd, int scr, int width, int height, double a, double f){
		id = s;
		moveType = m;
		health = hp;
		speed = spd;
	...
	//Allows for it to go diagonally
	public void goDiagonal()
	{
		...
		if(dx >= 1 || dx <= -1)
		{
			if( (this.getX() + (int)dx > wFrame - this.getWidth()) || (this.getX() + (int)dx < 0) )
			{
				direction *= -1;
				//dx *= -1;
			}
	...
	//Returns if the enemy object is outside of bounds
	public boolean inBounds()
	{
		return (this.getY() > hFrame);
	}
